...
std::string GetExtension(const std::string& path) {
	int idx = path.rfind('.');
	auto idxAdd = idx + 1;
	return path.substr(idxAdd, path.length() - idx - 1);
}
...
ID3D12Resource* CreateWhiteTexture() {
	...
	recDesc.SampleDesc.Quality = 0;
	recDesc.MipLevels = 1;
	recDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
	recDesc.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN;
	recDesc.Flags = D3D12_RESOURCE_FLAG_NONE;
...
#pragma pack()//1バイトパッキング解除
	...
	const CD3DX12_RESOURCE_DESC& descVertices = CD3DX12_RESOURCE_DESC::Buffer(vertices.size());
	result = _dev->CreateCommittedResource(
		&heapProps,
		D3D12_HEAP_FLAG_NONE,
		&descVertices,
	...
	//OKだと思います。
	const CD3DX12_RESOURCE_DESC& descIndices = CD3DX12_RESOURCE_DESC::Buffer(indices.size() * sizeof(indices[0]));
	result = _dev->CreateCommittedResource(
		&heapProps,
		D3D12_HEAP_FLAG_NONE,
		&descIndices,
	...
	const CD3DX12_RESOURCE_DESC& descMaterial = CD3DX12_RESOURCE_DESC::Buffer(materialBuffSize * materialNum);
	result = _dev->CreateCommittedResource(
		&heapProps,
		D3D12_HEAP_FLAG_NONE,
		&descMaterial,
	...
	//テクスチャ1つ目(↑のマテリアルとペア)
	descTblRange[2].NumDescriptors = 1;//テクスチャ４つ(基本とsphとspaとトゥーン)
	...
	const CD3DX12_RESOURCE_DESC& desc = CD3DX12_RESOURCE_DESC::Buffer((sizeof(MatricesData) + 0xff) & ~0xff);
	result = _dev->CreateCommittedResource(
		&heapProps,
		D3D12_HEAP_FLAG_NONE,
		&desc,
	...
	while (true) {
		...
		auto cbvsrvIncSize = _dev->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV) * 2; //２倍
...
std::string GetTexturePathFromModelAndTexPath(const std::string& modelPath, const char* texPath) {
	...
	//int型に代入しているのは見つからなかった場合はrfindがepos(-1→0xffffffff)を返すため
	int pathIndex1 = 0;
	if (modelPath.rfind('/') != std::string::npos)
	{
		pathIndex1 = modelPath.rfind('/');
	}
	int pathIndex2 = 0;
	if (modelPath.rfind('\\') != std::string::npos)
	{
		pathIndex2 = modelPath.rfind('\\');
	}
	auto pathIndex = max(pathIndex1, pathIndex2);
...
#pragma pack()//1バイトパッキング解除
	...
	//設定は、バッファのサイズ以外頂点バッファの設定を使いまわして
	//OKだと思います。
	const CD3DX12_RESOURCE_DESC& resourceDescBuffer = CD3DX12_RESOURCE_DESC::Buffer(indices.size() * sizeof(indices[0]));
	result = _dev->CreateCommittedResource(
		&heapProps,
		D3D12_HEAP_FLAG_NONE,
		&resourceDescBuffer,
	...
	materialBuffSize = (materialBuffSize + 0xff) & ~0xff;
	ID3D12Resource* materialBuff = nullptr;
	const CD3DX12_RESOURCE_DESC resourceDescBufferSize = CD3DX12_RESOURCE_DESC::Buffer(materialBuffSize * materialNum);
	result = _dev->CreateCommittedResource(
		&heapProps,
		D3D12_HEAP_FLAG_NONE,
		&resourceDescBufferSize,
		D3D12_RESOURCE_STATE_GENERIC_READ,
		nullptr,
		IID_PPV_ARGS(&materialBuff)
	);
	...
	ID3D12Resource* constBuff = nullptr;
	const CD3DX12_RESOURCE_DESC& resourceDescSceneData = CD3DX12_RESOURCE_DESC::Buffer((sizeof(SceneData) + 0xff) & ~0xff);
	result = _dev->CreateCommittedResource(
		&heapProps,
		D3D12_HEAP_FLAG_NONE,
		&resourceDescSceneData,
		D3D12_RESOURCE_STATE_GENERIC_READ,
		nullptr,
		IID_PPV_ARGS(&constBuff)
	);
	...
	while (true) {
		...
		//バックバッファのインデックスを取得
		auto bbIdx = _swapchain->GetCurrentBackBufferIndex();
		const CD3DX12_RESOURCE_BARRIER& resourceBarrier = CD3DX12_RESOURCE_BARRIER::Transition(_backBuffers[bbIdx],
		...
		const CD3DX12_RESOURCE_BARRIER resourceBarrierTran = CD3DX12_RESOURCE_BARRIER::Transition(_backBuffers[bbIdx],
			D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT);
		_cmdList->ResourceBarrier(1, &resourceBarrierTran);
